...
def cluster_accretion_loops(ID_voro_dict, threshold_arr,ID_core_arr,neighbor_ptr,neighbor_ids,VoroXYZ,VoroVol,zDens):
    ...
    Xcm = np.empty((Num_vds,Nthresholds,3))
    Vol_interp = np.empty((Num_vds,Nthresholds))
    Ncells_in_void = np.empty((Num_vds,Nthresholds))
    ell_eigenvalues = np.empty((Num_vds,Nthresholds,3))
    ell_eigenvectors = np.empty((Num_vds,Nthresholds,3,3))
...
def voronoi_threshold(threshold,ID_core_arr,neighbor_ptr,neighbor_ids,VoroXYZ,VoroVol,zDens,nthreads=-1,verbose=True):
    ...
    numPart = int(VoroXYZ.shape[0]*np.max(threshold_arr))
    Num_vds = ID_core_arr.shape[0]
    for iv in range(Num_vds):
        ID_voro_dict[iv] = np.zeros(numPart,dtype=np.int_)
    t0 = time.time()
    verboseprint('\n    computation started',flush=True)
    Xcm, Vol_interp, Ncells_in_void, ell_eigenvalues, ell_eigenvectors = cluster_accretion_loops(
        ID_voro_dict, threshold_arr,ID_core_arr,neighbor_ptr,neighbor_ids,VoroXYZ,VoroVol,zDens)

    dt = time.time() - t0
    verboseprint("    done,",StrHminSec(dt),'\n',flush=True)
        
    return ID_voro_dict, Xcm, Vol_interp, Ncells_in_void, ell_eigenvalues, ell_eigenvectors
